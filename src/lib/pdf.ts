...
// Re-exported for every existing caller that imports these from here —
// moved to their own module (see its doc comment) so `searchWorker.ts` can
// use them without pulling pdf.js into the worker bundle.
export { buildSearchRegex, findPdfMatches, type PdfMatch } from './pageSearchCore'
